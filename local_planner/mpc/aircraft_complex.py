...
Pitch is for speed and throttle is for altitude
...
class AirplaneSimpleModel():
    ...
    def define_controls(self):
        ...
        self.u_thrust = ca.SX.sym('u_thrust')
        self.u_load_factor = ca.SX.sym('u_load_factor')
    ...
    def set_state_space(self):
        ...
        self.x_fdot = self.v_f * ca.cos(self.theta_f) * ca.cos(self.psi_f) 
        self.y_fdot = self.v_f * ca.cos(self.theta_f) * ca.sin(self.psi_f)
        self.z_fdot = -self.v_f * ca.sin(self.theta_f)
                
        self.theta_fdot = (self.g/self.v_f) * \
            ((self.u_load_factor * ca.cos(self.u_phi)) - ca.cos(self.psi_f))
        
        self.psi_fdot = (self.g/self.v_f) * \
            (self.u_load_factor * ca.sin(self.u_phi) / ca.cos(self.theta_f))

        self.drag = 0.5 * Config.RHO * Config.C_DRAG_P * Config.S * self.v_f**2
        self.v_dot = self.u_thrust - self.drag / (Config.MASS - (self.g * ca.sin(self.theta_f)))

        # self.v_fdot = self.u_thrust - (self.g/self.v_f) * \
